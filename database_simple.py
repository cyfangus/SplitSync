...
def load_data(current_user=None):
    """Load data from Supabase - simplified version."""
    supabase = init_connection()
    if not supabase:
        return {'users': [], 'events': []}

    try:
        # Load users (public info only)
        ...
        for event in events_raw:
            ...
            for m in members_raw:
                if m['event_id'] == event_id:
                    event['members'].append(m['username'])
                    event['roles'][m['username']] = m['role']
            
            # Add expenses
            event['expenses'] = []
            for exp in expenses_raw:
                if exp['event_id'] == event_id:
                    exp['involved'] = [p['username'] for p in participants_raw if p['expense_id'] == exp['id']]
                    exp['settled'] = bool(exp.get('settled', False))
                    event['expenses'].append(exp)
            
            # Add settlements
            event['settlements'] = [s for s in settlements_raw if s['event_id'] == event_id]
